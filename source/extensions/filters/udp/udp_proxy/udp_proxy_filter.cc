...
UdpProxyFilter::ClusterInfo::createSession(Network::UdpRecvData::LocalPeerAddresses&& addresses,
  ...
  if (!cluster_info_->resourceManager(Upstream::ResourcePriority::Default)
           .connections()
           .canCreate()) {
    ENVOY_LOG(debug, "cannot create new connection.");
    cluster_info_->trafficStats()->upstream_cx_overflow_.inc();
    return nullptr;
  }
...
void UdpProxyFilter::ActiveSession::onSessionComplete() {
  ...
  cluster_.cluster_info_->resourceManager(Upstream::ResourcePriority::Default).connections().dec();
